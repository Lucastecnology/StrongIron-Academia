...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (addDietForm) {
    addDietForm.addEventListener('submit', async (e) => {
      ...
      const foods = Array.from(document.querySelectorAll('#foods-list .food-entry')).map(entry => {
      ...
      try {
        const response = await fetch(`http://localhost:3800/diets/${email}`, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
        });
        ...
        const saveResponse = await fetch(`http://localhost:3800/diets/${email}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        alert('Erro ao adicionar dieta. Verifique o console para mais detalhes.');
      }
    });
  }
  ...
  if (dietsBody) {
    ...
    fetch(`http://localhost:3800/diets/${email}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(response => {
      ...
      .then(diets => {
        ...
        diets.forEach((diet, index) => {
          ...
          const foodsList = diet.foods.map((food, foodIndex) => `
            <div class="food-item">
      ...
      .catch(error => {
        ...
        alert('Erro ao carregar dietas. Verifique o console para mais detalhes.');
      });
  }
  ...
  if (addWeightForm) {
    addWeightForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch(`http://localhost:3800/weights/${email}`, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
        });
        ...
        const saveResponse = await fetch(`http://localhost:3800/weights/${email}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        alert('Erro ao adicionar peso. Verifique o console para mais detalhes.');
      }
    });
  }
  ...
  if (weightsBody && weightChartCanvas) {
    ...
    fetch(`http://localhost:3800/weights/${email}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(response => {
      ...
      .catch(error => {
        ...
        alert('Erro ao carregar pesos. Verifique o console para mais detalhes.');
      });
  }
  ...
  if (addWorkoutForm) {
    addWorkoutForm.addEventListener('submit', async (e) => {
      ...
      const exercises = Array.from(document.querySelectorAll('#exercises-list .exercise-entry')).map(entry => {
      ...
      try {
        const response = await fetch(`http://localhost:3800/workouts/${email}`);
        ...
        await fetch(`http://localhost:3800/workouts/${email}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  if (workoutsBody) {
    ...
    fetch(`http://localhost:3800/workouts/${email}`)
        ...
        if (users.length === 0) {
          ...
          users.forEach(user => {
            ...
            const workoutsList = user.workouts.map(w => `
              ${w.day} (${w.title}):<br>
              ${w.exercises.map(ex => `- ${ex.name}: ${ex.sets} séries de ${ex.reps} repetições`).join('<br>')}
            `).join('<br><br>');
            row.innerHTML = `
              <td>${user.email}</td>
              <td>${user.password}</td>
...
function saveDiet(index) {
  ...
  const foods = Array.from(document.querySelectorAll(`#edit-foods-${index} .food-item`)).map((entry, foodIndex) => {
...
function saveWorkout(index) {
  ...
  const exercises = Array.from(document.querySelectorAll(`#edit-exercises-${index} .exercise-item`)).map((entry, exIndex) => {
